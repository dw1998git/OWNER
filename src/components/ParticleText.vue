...
const plainText = computed(() => {
  ...
  return props.stripHtml ? text.replace(/<[^>]+>/g, '') : text
})
...
onMounted(() => {
  ...
  let height = 0
  let dpr = 1
  let isVisible = !props.visibleOnly
  ...
  const sampleText = async () => {
    ...
    const lineHeight = resolvedSize * props.lineHeight
    ...
    const lineMetrics = lines.map(line => {
      ...
      return { text: line, width: left + right, height: ascent + descent, left, ascent }
    })

    const maxLineWidth = Math.max(...lineMetrics.map(l => l.width))
    const totalTextHeight = lineMetrics.reduce((sum, l) => sum + l.height, 0) + lineHeight * (lineMetrics.length - 1)

    offscreen.width = Math.max(1, maxLineWidth + padding * 2)
    offscreen.height = Math.max(1, totalTextHeight + padding * 2)
    ...
    lineMetrics.forEach((line, i) => {
      const y = padding + line.ascent + i * lineHeight
      const x = padding + (maxLineWidth - line.width) / 2 + line.left
  ...
  if (props.visibleOnly && 'IntersectionObserver' in window) {
    visibilityObserver = new IntersectionObserver(
      entries => {
        entries.forEach(entry => {
          const wasVisible = isVisible
          isVisible = entry.isIntersecting
          if (isVisible && !wasVisible && animationFrame === null) {
            animationFrame = window.requestAnimationFrame(render)
          }
        })
      },
