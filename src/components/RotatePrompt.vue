<template>
  ...
  <transition name="fade">
    <div class="rotate-overlay" v-if="showFullOverlay" @click="dismissFullOverlay">
  ...
  <!-- 后续小提示 -->
  <transition name="fade">
    <div class="rotate-mini" v-if="showMiniToast">
      <svg class="rotate-mini-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        <polyline points="23 4 23 10 17 10"></polyline>
        <path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"></path>
      </svg>
      <span>横屏浏览效果更佳</span>
...
const showFullOverlay = ref(false)
const showMiniToast = ref(false)
const hasSeenOverlay = ref(false)
let timer = null

function check() {
  const isMobile = /Mobi|Android|iPhone/i.test(navigator.userAgent)
  if (!isMobile) {
    showFullOverlay.value = false
