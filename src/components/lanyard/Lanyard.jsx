...
function Band({
  ...
  // 绳子减半：三段绳段 1 → 0.5；卡片锚点保持 1.5，保证绳带末端与卡片绳环始终连接
  ...
  useFrame((state, delta) => {
    // 限制单帧步长：切回标签页或首帧卡顿时 delta 会突增，导致物理过冲、挂绳抽搐
    ...
    // 限制回正力矩，避免大角度时反复过冲造成卡片抖动
    const yCorrection = Math.max(-1.5, Math.min(1.5, -rot.y * 0.25));
    card.current.setAngvel({ x: ang.x, y: ang.y + yCorrection, z: ang.z });
  });
